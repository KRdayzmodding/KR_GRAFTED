...
В своём проекте так делать не обязательно: по умолчанию `graft_plugin` кладёт объявления
рядом с DLL, а в мод их забирают перед упаковкой PBO. Но тогда они живут только в каталоге
сборки — а его чистят, и мод остаётся без них.
