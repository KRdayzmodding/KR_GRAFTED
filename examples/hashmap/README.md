...
{
    private proto void NativeDispose_EXAMPLE_HASHMAP();
    ...
    void ~CppHashMap()
    {
        NativeDispose_EXAMPLE_HASHMAP();   // состояние объекта на стороне C++
    }
}
```

Восемь `proto` — это ровно восемь `.method(GRAFT_METHOD(...))` из C++, и ни одного лишнего
знака: имена, типы и порядок взяты из собранной DLL. Шаблонный синтаксис
`<Class K, Class V>` — из строки `"CppHashMap<Class K, Class V>"` в привязке. Деструктор
генератор дописывает сам, потому что за объектом стоит состояние в C++.
...
движок зовёт `~CppHashMap`, тот `NativeDispose_EXAMPLE_HASHMAP`, и `std::unordered_map`
в C++ снесён. Последняя строчка лога (`count=0` у новой таблицы) — ровно про это.

Суффикс `_EXAMPLE_HASHMAP` в имени освобождения берётся из `NAME` плагина: два плагина,
держащие состояние на одном скриптовом классе, не должны спорить за один метод.
