...
`NAME` попадает в два места сразу: имя DLL и паспорт плагина.
...
- **Состояние объекта освобождается само.** Экземпляр класса C++ живёт ровно столько,
  сколько живёт скриптовый объект: о его смерти хост узнаёт от движка (тем же путём,
  каким чистит себя ванильный `map`), поэтому в скрипте для этого нет ни строчки.
