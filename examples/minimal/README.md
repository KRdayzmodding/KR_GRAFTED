...
`<мод>/graft/` или из `<каталог игры>/graft/`. Плагинов может быть сколько угодно, они
не мешают друг другу и могут быть собраны разными компиляторами.

```
DayZ.exe
 └─ dwmapi.dll                      хост, один на игру: graft install <каталог игры>
      ├─ @MYMOD/graft/MYMOD.grafted.dll
      ├─ @OTHER/graft/OTHER.grafted.dll
      └─ graft/EXAMPLE_GRAFT.grafted.dll     общая папка, для разработки
```
