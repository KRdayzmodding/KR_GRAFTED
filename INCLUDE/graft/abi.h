...
typedef struct graft_host_api {
    ...
    // За объектом стоит состояние на стороне плагина — сообщи, когда движок его
    // разрушит. Хост подменяет объекту нулевой слот его C++ таблицы: тем же слотом
    // чистят себя array/map/set, поэтому в скрипте объявлять нечего.
    void (*watch_object)(void* self, void (*forget)(void*));
    // Импл ГЛОБАЛЬНОЙ функции движка по имени: у неё нет класса, а значит и пути через
