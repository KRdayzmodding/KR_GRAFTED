...
| Модель ветвления: один ствол `main`, ветка под задачу, удаление после мержа | `dev` заведена и снята за день, ruleset для неё удалён |
...
| ~~Решить про approvals~~ — оставлены 0 по решению владельца | в репозитории **три админа** (`warcat06`, `HiimOrange`, `6wingSerap`), а не один. `required_approving_review_count: 1` перестал быть самоблокировкой — см. §1 |
...
**Ветвление**: **только `main`**, долгоживущих веток нет. Фичи — `feat/<тема>`, фиксы —
`fix/<тема>`, ветка живёт до мержа и удаляется. Ветка `dev` была заведена и через день
снята — см. §7: у AUI такая ветка умерла естественной смертью, и её труп до сих пор
собирает PR-ы, которые некуда влить.
...
**Сделано в `v0.1.0`.** До него в README стоял `TAG main`, то есть все пользователи
сидели на движущейся ветке и первый же бамп ABI сломал бы всех разом. Теперь:

1. Тег `v0.1.0` стоит, релиз опубликован, `project(graft VERSION)` поднят до `0.1.0`.
2. В README (обоих) — `TAG v0.1.0` и врезка «`main` — ветка разработки самого graft,
   мод держи на теге».
...
с внятным текстом, если релиз с такой версией уже выпущен. Поэтому **сразу после
публикации релиза номер поднимают** — иначе первый же пуш в `main` красит джобу `release`.

Ручного в релизе остаётся четыре шага, и все они в одном PR:

| Шаг | Где |
|---|---|
| Поднять номер версии | `project(graft VERSION ...)` в `CMakeLists.txt` |
| Раздел релиза + строка «проверено на сборке DayZ» | `CHANGELOG.md` |
| `TAG vX.Y.Z` в примере проекта мода | `README.md`, `README.en.md` |
| Опубликовать черновик, который собрал CI | `gh release create` |

Бейдж версии в шапке обоих README менять не надо: `shields.io/github/v/release`
спрашивает GitHub сам. `TAG` в примере — надо, он не может быть плавающим по смыслу.
...
| Ветка `dev` | не появится: пробовали, откатили — §7 |
...
### 7.1 Ветвление: почему `dev` прожила один день

Документация AUI описывает git-flow: `master` — ветка релиз-кандидатов, фичи растут из
неё в `feat/`. Про `develop` в тексте ни слова, хотя ветка есть и воркфлоу до сих пор
триггерятся на неё. Данные говорят иначе:

| Что смотрели | Что видно |
|---|---|
| `compare/master...develop` | `ahead 0, behind 21`; последний коммит `develop` — 17.05.2026, называется **«migration to master branch»** |
| Базовые ветки последних 60 влитых PR | 41 в `develop` — **все до 16 мая**; 19 в `master` — все после |
| Открытые PR | 10 до сих пор целятся в мёртвую `develop` |
| Ветки | 43 штуки, 35 из них `feat/*` — после мержа не удаляются |
| Форма истории `master` | последние 20 коммитов однородительские, заголовки с `(#NNN)` — squash, линейно |
| `rules/branches/master` и `/develop` | **пусто**: ни обязательного PR, ни обязательных проверок |

Проект пять лет жил на двух ветках и весной 2026 схлопнулся в одну. Труп `develop` при
этом не исчез, а продолжает собирать PR-ы, которые некуда влить, — и воркфлоу до сих пор
перечисляют её в триггерах.

**Отсюда решение:** у нас один ствол `main`, ветка под задачу, удаление после мержа.
`dev` была заведена и снята в тот же день вместе со своим ruleset'ом. Из формы истории
мы с AUI совпадаем полностью, а по гарантиям строже: у них ничто не мешает влить красный
PR или запушить в `master` напрямую — держится на том, что мейнтейнер один и он же автор.
